...
r_min_vec=$(cat "$PARAM_STATS_R_MIN_VEC")
r_max_vec=$(cat "$PARAM_STATS_R_MAX_VEC")
r_vecs=$(cat "$PARAM_STATS_R_VECS")
r_segments=$(cat "$PARAM_STATS_R_SEGMENTS")
r_decomp_size=$(cat "$PARAM_STATS_R_DECOMP_SIZE")
r_comp_size=$(cat "$PARAM_STATS_R_COMP_SIZE")
r_mem_usage=$(cat "$PARAM_STATS_R_MEM_USAGE")
...
w_min_vec=$(cat "$PARAM_STATS_W_MIN_VEC")
w_max_vec=$(cat "$PARAM_STATS_W_MAX_VEC")
w_vecs=$(cat "$PARAM_STATS_W_VECS")
w_segments=$(cat "$PARAM_STATS_W_SEGMENTS")
w_decomp_size=$(cat "$PARAM_STATS_W_DECOMP_SIZE")
w_comp_size=$(cat "$PARAM_STATS_W_COMP_SIZE")
w_mem_usage=$(cat "$PARAM_STATS_W_MEM_USAGE")
...
all_min_vec=$(min "$r_min_vec" "$w_min_vec")
all_max_vec=$(max "$r_max_vec" "$w_max_vec")
all_vecs=$(add "$r_vecs" "$w_vecs")
...
all_mem_usage=$(add "$r_mem_usage" "$w_mem_usage")
...
r_avg_vec=$(div "$r_decomp_size" "$r_vecs" "3")
r_avg_segment=$(div "$r_decomp_size" "$r_segments" "3")
r_avg_mem=$(div "$r_mem_usage" "$r_reqs_success" "3")
...
w_avg_vec=$(div "$w_decomp_size" "$w_vecs" "3")
w_avg_segment=$(div "$w_decomp_size" "$w_segments" "3")
w_avg_mem=$(div "$w_mem_usage" "$w_reqs_success" "3")
...
all_avg_vec=$(div "$all_decomp_size" "$all_vecs" "3")
all_avg_segment=$(div "$all_decomp_size" "$all_segments" "3")
all_avg_mem=$(div "$all_mem_usage" "$all_reqs_success" "3")
...
if [ "$PRINT_READ" -eq 1 ]; then
	...
	output+="\tmin_vec: $r_min_vec\n"
	output+="\tmax_vec: $r_max_vec\n"
	output+="\tvecs: $r_vecs\n"
	output+="\tsegments: $r_segments\n\n"
	output+="\tavg_block: $r_avg_block\n"
	output+="\tavg_vec: $r_avg_vec\n"
	output+="\tavg_segment: $r_avg_segment\n\n"
	output+="\tavg_mem: $r_avg_mem\n"
...
if [ "$PRINT_WRITE" -eq 1 ]; then
	...
	output+="\tmin_vec: $w_min_vec\n"
	output+="\tmax_vec: $w_max_vec\n"
	output+="\tvecs: $w_vecs\n"
	output+="\tsegments: $w_segments\n\n"
	output+="\tavg_block: $w_avg_block\n"
	output+="\tavg_vec: $w_avg_vec\n"
	output+="\tavg_segment: $w_avg_segment\n\n"
	output+="\tavg_mem: $w_avg_mem\n"
...
if [ "$PRINT_ALL" -eq 1 ]; then
	...
	output+="\tmin_vec: $all_min_vec\n"
	output+="\tmax_vec: $all_max_vec\n"
	output+="\tvecs: $all_vecs\n"
	output+="\tsegments: $all_segments\n\n"
	output+="\tavg_block: $all_avg_block\n"
	output+="\tavg_vec: $all_avg_vec\n"
	output+="\tavg_segment: $all_avg_segment\n\n"
	output+="\tavg_mem: $all_avg_mem\n"
